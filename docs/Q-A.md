...
| 日期 | 内容 | 对应版本 |
|------|------|----------|
| 2026-01-08 | 从臂静止抖动问题修复 | [v0.2.141](RELEASE.md#v0.2.141) |
...
**对应版本：** [v0.2.141](RELEASE.md#v0.2.141)
...
- 抖动表现为"抽一下抽一下"的间歇性突变
- 硬件检查正常，无机械故障
- 对应主臂 joint_0 的关节抖动最明显

**影响范围：**
- 所有从臂关节都可能受影响
- 对应小范围关节（如 joint_0）的抖动最明显
- 影响数据采集质量和系统稳定性

### 根本原因

**传感器噪声 + 无死区 + PID 放大 = 持续抖动**

1. **传感器噪声**：
   - 主臂位置传感器存在 ±1-2 PWM 单位的读取噪声
   - 即使主臂完全静止，读取值也会在 1500 ± 2 之间波动
   - 这些微小波动被当作"新的目标位置"发送给从臂

2. **无位置死区**：
   - 从臂电机对任何位置误差都会响应
   - 即使只有 1-2 个单位的误差也会触发电机运动
   - 没有"容忍范围"来过滤微小误差
...
4. **震荡循环**：
   ```
   传感器噪声 → PID 响应 → 电机移动 → 超调 → 反向修正 → 再次超调 → 持续震荡
   ```

5. **小范围关节特别敏感**：
   - joint_0 范围：205 PWM 单位
   - 其他关节范围：1000-2000 PWM 单位
   - 相同的 2 单位噪声在 joint_0 上占比 0.98%，在其他关节上只占 0.2%
   - joint_0 对噪声的敏感度是其他关节的 **5 倍**
...
**修改位置 - configure_follower 函数（line 67-76）：**
```python
def configure_follower(bus: FeetechMotorsBus) -> None:
    with bus.torque_disabled():
        ...
        for motor_name, motor in bus.motors.items():
            ...
            bus.write("CW_Dead_Zone", motor, 4)   # 顺时针死区
            bus.write("CCW_Dead_Zone", motor, 4)  # 逆时针死区
```
...
| 迭代 | 滤波器 alpha | 死区大小 | 效果 |
|------|-------------|---------|------|
| 初始状态 | 无 | 无 | 持续抖��� |
| 尝试 1 | 无 | 2 | 抖动减少，仍有抽动 |
| 尝试 2 | 0.3 | 2 | 效果好很多，偶尔抽动 |
| **最终方案** | **0.2** | **4** | **完全稳定** |

**参数说明：**
- `filter_alpha = 0.2`：经验最优值，无需调整
- `Dead_Zone = 4`：适合 4096 分辨率电机，无需调整

---

#### 其他可选方案（不推荐）

如果不想使用双层过滤，可以考虑以下单一方案：

**方案 A：仅使用死区**
- 优点：实现简单，无延迟
- 缺点：需要更大的死区值（5-6），精度损失更大

**方案 B：仅使用滤波器**
- 优点：保持精度
- 缺点：需要更强的滤波（alpha=0.1），延迟更大
