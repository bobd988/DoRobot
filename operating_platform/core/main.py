...
def record_loop(cfg: ControlPipelineConfig, daemon: Daemon):
    ...
    # Always clear existing data and start fresh to avoid corrupted/incomplete dataset issues
    # This prevents errors like missing meta/tasks.jsonl from previous interrupted sessions
    resume = False
    if any(target_dir.iterdir()):  # 检查目录是否非空
        import shutil
        logging.warning(f"Clearing existing data in {target_dir} to start fresh")
