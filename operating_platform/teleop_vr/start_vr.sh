#!/bin/bash
# VR 模块快速启动脚本
# 位置: /home/demo/Public/DoRobot/operating_platform/teleop_vr/start_vr.sh
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}DoRobot VR 远程操控系统启动脚本${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
if [ ! -e "/dev/ttyACM0" ]; then
    echo -e "${YELLOW}警告: 未检测到机械臂设备 (/dev/ttyACM0)${NC}"
    echo -e "${YELLOW}系统将启动，但无法控制真实硬件${NC}"
    echo -e "${YELLOW}如需使用真实硬件，请连接机械臂后重启${NC}"
else
    echo -e "${GREEN}✓ 机械臂设备已连接${NC}"
fi

echo ""
echo -e "${YELLOW}[2/4] 启动 Dora 守护进程...${NC}"
dora up
echo -e "${GREEN}✓ Dora 守护进程已启动${NC}"

echo ""
echo -e "${YELLOW}[3/4] 启动 VR 数据流...${NC}"
dora start dora_vr_teleoperate.yml

echo ""
echo -e "${YELLOW}[4/4] 获取系统信息...${NC}"
sleep 2
